...
model = VipLlavaForConditionalGeneration.from_pretrained("llava-hf/vip-llava-7b-hf", torch_dtype=torch.float16).to('cuda')
...
for pic in tqdm(list(os.walk('../Datasets/COCO/train2014/train2014/'))[0][2]):
    ...
    answer = processor.decode(generate_ids[0][len(inputs["input_ids"][0]):], skip_special_tokens=True)
