...
def login_to_bing(username, password):
    ...
    try:
        ...
        WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, ".signin_content > a:nth-child(1)"))).click()
...
def get_credits(email):
    driver = get_or_create_driver("https://www.bing.com/images/create")
    try:
        credit = int(WebDriverWait(driver, 10).until(EC.element_to_be_clickable((By.ID, 'token_bal'))).text)
        return credit
    except TimeoutException:
...
def download_images(user, prompt, path, max_images=5):
    ...
    try:
        ...
        WebDriverWait(driver, 30).until(EC.presence_of_all_elements_located((By.XPATH, '//img[@class="mimg"]')))
