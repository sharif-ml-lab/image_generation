...
def enhance_prompt(base_prompt, body_template, model):
    try:
        ...
        body["model"] = model
        body["messages"][-1]["content"] = f'Enhance the following prompt by adding diverse elements while maintaining the core scenario: "{base_prompt}". Focus on diversity in roles, activities, and settings. Output a fully detailed prompt in a string format. \"\"'
    ...
    for _ in range(3):
        ...
        if prompt is not None:
            ...
            if score > 0.8:
                print("ACCEPTED")
                return prompt
    logging.warning(f"Failed to generate an enhanced prompt after 3 attempts for base prompt: {base_prompt}")
    return False